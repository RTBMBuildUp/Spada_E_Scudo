...
trait Action {
  def execute[T <: Creature](creature: T, map: Map[String, Creature]): Map[String, Creature]
}

object Choices {
  case object Attack extends Action with Identifilable {
    override def execute[T <: Creature](creature: T, participantMap: Map[String, Creature]): Map[String, Creature] = {
